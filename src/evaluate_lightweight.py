#!/usr/bin/env python3
"""
Evaluate Lightweight System with proper HOTA metrics
"""
...
from multistage_fixed_lightweight import LightweightMultiStageFusion
...
def evaluate():
    ...
    for video_id in video_ids:
        ...
        for frame_file in frame_files:
            ...
            # Process with lightweight system
    ...
    logger.info("\n" + "="*60)
    logger.info("LIGHTWEIGHT SYSTEM RESULTS")
    logger.info("="*60)
    ...
    logger.info(f"  Lightweight (anchors): HOTA={avg_hota:.3f}")

    improvement = (avg_hota - 0.187) / 0.187 * 100
    if improvement > 0:
        logger.info(f"  ✅ Improvement: +{improvement:.1f}% over optimized")
    else:
        logger.info(f"  ❌ Decrease: {improvement:.1f}% from optimized")
