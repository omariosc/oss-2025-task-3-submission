#!/usr/bin/env python3
"""
Evaluate YOLO + Anchors System with proper HOTA metrics
"""
...
def evaluate():
    ...
    logger.info("\n" + "="*60)
    logger.info("YOLO + ANCHOR FUSION RESULTS")
    logger.info("="*60)
    ...
    logger.info(f"  Original Multi-Stage:  HOTA=0.127")
    logger.info(f"  Optimized (K-means):   HOTA=0.187")
    logger.info(f"  Lightweight (anchors): HOTA=0.181")
    logger.info(f"  YOLO + Anchors:        HOTA={avg_hota:.3f}")

    improvement_over_best = (avg_hota - 0.187) / 0.187 * 100
    if improvement_over_best > 0:
        logger.info(f"\n✅ NEW BEST! Improvement: +{improvement_over_best:.1f}% over previous best")
    else:
        logger.info(f"\n❌ No improvement: {improvement_over_best:.1f}% from best")

    # Detailed analysis
    logger.info("\nDetailed Performance:")
    logger.info(f"  Detection Accuracy (DetA): {avg_deta:.3f}")
    logger.info(f"    - This measures how well we detect objects")
    logger.info(f"    - Target: > 0.5 for good performance")
    logger.info(f"  Association Accuracy (AssA): {avg_assa:.3f}")
    logger.info(f"    - This measures tracking consistency")
    logger.info(f"    - Current: {'Good' if avg_assa > 0.6 else 'Needs improvement'}")
